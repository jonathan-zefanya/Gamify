...
@section('content')
    <div class="container">
        <div class="pagetitle mt-20">
            <h4 class="mb-1">@lang('add fund')</h4>
            <nav>
                <ol class="breadcrumb">
                                        ...
                                        @foreach($gateways as $key => $gateway)
                                            <li class="item">
                                                <input class="form-check-input methodId" type="radio"
                                                       ...
                                                       id="wallet{{$key}}"
                                                    {{ (old('gateway_id') == $gateway->id || $loop->first) ? 'checked' : '' }}>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function checkAmount(amount, selectedCurrency, selectGateway) {
                console.log(amount, selectedCurrency, selectGateway)
                $.ajax({
                    method: "GET",
            ...
            function showCharge(response, currency) {
                $('.showCharge').html(`${response.charge} ${response.currency}`)
                $('#payable').html(`${response.payable_amount} ${response.currency}`)
                $('#fundingAmount').html(`${baseCurrency}${response.payable_amount_baseCurrency}`)
            }
